...
**Open Standard Proposal — Draft v0.45 — June 2026**
...
v0.45 is a **descriptive, additive** revision. Per the Governance Addendum (§1),
the Conceptual Framework is descriptive and not under normative version control —
*only* the Core Invariants (§11.4) are normative. **v0.45 changes no Core Invariant
and changes no normative requirement.** v0.44 is retained verbatim as a snapshot.

What is new:

- **An optional Profile layer (§14).** Three companion documents, distilled from the
  first serious landing application (ELMS — a collective, AI-driven,
  blockchain-settled liquidity pool) and folded back upstream, are accepted:
  - **ACP** — Agent Conformance Profile (AATP-RFC-0001): the named, testable
    "is this a valid AATP agent?" predicate, with a three-tier conformance model.
  - **LAEP** — Ledger Anchoring & Evidence-Persistence Profile (AATP-RFC-0002): how a
    *privately sealed* chain (Invariant 3) becomes a *publicly non-repudiable,
    durable* one, with the **oracle gap** fenced explicitly.
  - **Finance / Valuation Profile** (AATP-RFC-0003): a *sketch* naming the
    investment and valuation extension decision points (§4.2.1).
  Each profile is **normative-optional** (an implementation conforms to AATP without
  adopting any of them) and **rides existing invariants without adding one**.
- **A non-normative Operating-Cost report** (`docs/AATP-Operating-Cost-Report-v0.1`):
  the marginal cost of operating under AATP is bounded by the same design
  disciplines the standard already prescribes (batching, model tiering, graduated
  profiles); cost is not an adoption barrier.
- **The "Merkle anchoring — scope and mandatory status to be determined based on
  implementation experience" item (v0.44 Roadmap, Future)** is resolved by LAEP: the
  implementation experience now exists. Anchoring is **optional in v0.x and
  carrier-neutral**; the anchored records form their own sealed chain (LAEP §3.2);
  two real-chain field notes from a public-testnet anchor are recorded in LAEP §11.
- **Batch mode (Principle 5)** is reaffirmed as *also* the primary on-chain *cost*
  lever, not only a scale lever (Operating-Cost report §6).

**Explicitly deferred (not in v0.45).** A native **multi-principal / delegation-graph**
primitive touches **Invariant 7** and is therefore a *successor-protocol* proposal
under Governance Addendum §3 — it is **not** folded here. Landing applications that
need collective principals continue to use the pool-as-single-principal wrapper, so
Invariant 7 holds unchanged.
...
## 14. Optional Profiles (v0.45)

AATP v0.45 introduces an **optional Profile layer**. A Profile is a *view* over the
seven Core Invariants (§11.4) sharpened into a named, testable artifact, or an
*optional externalization* that rides those invariants. **Profiles never add or
alter a Core Invariant** (a change to an invariant is a successor protocol, per
Governance Addendum §3). An implementation conforms to AATP without adopting any
Profile; Profiles define what *specific additional claims* (e.g. "Tier-1
conformant," "anchored") mean for implementations that choose to make them.

This is the same dividing line that keeps AATP neutral: AATP absorbs only what is
**cross-domain generic, about recording/verifying, and invariant-compatible**;
everything domain-specific, or about enforcing/controlling outcomes, stays in the
landing application. The Profiles below were distilled from ELMS (the first serious
landing application) and folded upstream — ELMS as the forcing function — taking
*only* the generic, recording/verifying, invariant-compatible part.

| Profile | RFC | Status | Rides on | One line |
| --- | --- | --- | --- | --- |
| **ACP** — Agent Conformance Profile | AATP-RFC-0001 | Accepted (normative-optional) | Inv 1–7, Principle 1 | The checkable "is this a valid AATP agent?" predicate; three-tier conformance model (impl / agent-instance / quality-out-of-scope). |
| **LAEP** — Ledger Anchoring & Evidence-Persistence Profile | AATP-RFC-0002 | Accepted (normative-optional) | Inv 2, 3, 5, 6, 7; Principles 2, 5 | Make a privately sealed chain publicly non-repudiable + durable; the **oracle gap** fenced; carrier-neutral; anchoring optional in v0.x. |
| **Finance / Valuation Profile** | AATP-RFC-0003 | Sketch (named, not frozen) | §4.2.1 extension points | Reusable investment + valuation decision points; attested basis, never a self-computed price. |
| **Operating-Cost & Token-Overhead Report** | — | Accepted (non-normative) | Principle 5 | Cost is not the adoption barrier; the cost levers are the standard's own disciplines. |

Profile documents live in `docs/profiles/` (and `docs/` for the non-normative
report). Each states its own scope fence, its relationship to the invariants, and
its open questions for Phase II. **No Core Invariant changes in v0.45.**